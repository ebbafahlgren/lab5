...
public class StoreState extends State {
	...
	private CreateCustomer skapaKund = new CreateCustomer();
...
//	} // flytta denna till view...
	...
	/**
	 * @param isOpen returnerar om butik öppen eller ej
	 */
	...
	public Customer createCustomer() {
		return skapaKund.createCustomer();
	}
	...
	/**
	 * @param customer om butiken Ã¤r stÃ¤ngd eller full blir kunden avvisad annars
	 *                 fÃ¥r den komma in
	 */
	public void addCustomer(Customer customer) {
		...
		} else if (getCurrentCustomers() < maxCustomers) {
			customer.setState(customerStatus.inStore);
		} else {
			System.out.println("butiken full");
	...
	public int getTurnedAwayCustomers() {
		int numTurnedAway = 0;
		for (int i = 0; i < customerList.size(); i++) {
			if (customerList.get(i).getState() == customerStatus.fullStore || customerList.get(i).getState() == customerStatus.walkedAway) {
	...
	public void update(Event thisEvent) {
		...
		if (thisEvent.getClass() != Stop.class) {
			if (store.getAvailableRegisters() == 0) {
				...
				if (thisEvent.getClass() == Arrival.class && store.isStoreOpen() == true) {
					store.setRegisterFreeTime(store.getRegisterFreetime());

				} else {
					store.setRegisterFreeTime(
							store.getRegisterFreetime() + timeBetweenEvent() * store.getAvailableRegisters());
				}

				if (thisEvent.getClass() == Pay.class && store.isStoreOpen() == false) {
					lastPaymentTime = currentTime;
				}
			}

			store.setTotCustomerQueueTime(
					store.getCustomerQueueTime() + timeBetweenEvent() * store.getTotCustomersInRegisterQueue());
		}
		setChanged();
		notifyObservers();
	}
